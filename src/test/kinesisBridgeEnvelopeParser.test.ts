...
describe('KinesisEnvelopeParser', () => {
    ...
    it('should return a parsed kinesis bridge envelope when called with valid payload', () => {
        ...
        const parser = new KinesisBridgeEnvelopeParser();
        const kinesisEnvelope : KinesisBridgeEnvelope = parser.parseData(payload);

        expect(kinesisEnvelope).to.be.an.instanceof(KinesisBridgeEnvelope);
        expect(kinesisEnvelope.headerVersion).to.equal(1);
        expect(kinesisEnvelope.timeUUID).to.equal('c64768f0-2623-e611-81a4-0e3b241d74e9');
        expect(kinesisEnvelope.sourceNameLength).to.equal(94);
        expect(kinesisEnvelope.sourceName).to.equal('e5d428b1-0e7f-4dbc-a1de-58bf69caee54');
        expect(kinesisEnvelope.sourcePropertiesLength).to.equal(203);
        expect(kinesisEnvelope.sourceProperties).to.deep.equal({
            SchemaVersion: 1,
            EntityType: 'device',
        ...
        expect(kinesisEnvelope.targetName).to.equal('xi/blue/v1/aa825c4d-eab6-4f72-8186-46f4bbf81b5f/d/e5d428b1-0e7f-4dbc-a1de-58bf69caee54/generic');
        expect(kinesisEnvelope.targetPropertiesLength).to.equal(128);
        expect(kinesisEnvelope.targetProperties).to.deep.equal({
            SchemaVersion: 1,
            EntityType: 'topic',
            TopicOwner:
            {
        ...
        expect(kinesisEnvelope.contentLength).to.equal(337);
        expect(kinesisEnvelope.contentBody).to.be.an.instanceof(Buffer);
        expect(JSON.parse(kinesisEnvelope.contentBody.toString('utf-8'))).to.deep.equal({
            accountId: 'aa825c4d-eab6-4f72-8186-46f4bbf81b5f',
